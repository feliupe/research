function data_track = display_data_and_track(data, HEIGHT, WIDTH, C,  trajectories, scale, start)
...
T = size(data, 3);
%data_track = zeros(HEIGHT, WIDTH, C, T);

for tt = start:T
  h = imagesc(reshape(data(:, :, tt), HEIGHT, WIDTH, C));
  ...
  data_track(:,:,:,tt) = frame2im(getframe);
  hold off;
  drawnow;
